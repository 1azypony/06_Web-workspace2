...
public class MemberInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String phone = request.getParameter("phone"); // 빈 문자열 "" 이 올수도 있다.
		String email = request.getParameter("email"); // 빈 문자열 "" 이 올수도 있다.
		String address = request.getParameter("address"); // 빈 문자열 "" 이 올수도 있다.
		...
		// 3) 요청 처리 (MemberService 클래스의 어떤 메소드를 호출할 때 전달값을 같이 넘겨서 결과 받기)
		int result = new MemberService().insertMember(m);
		
		// 4) 처리 결과를 가지고 사용자가 보게 될 응답 뷰 지정
		if(result > 0) { // 성공 => 메인페이지 (url 재요청 방식)
			
			// 우선적으로 session 에 현재 회원가입에 성공한 회원의 정보를 담아둘것
			...
			// 4_1) 응답페이지에서 필요로 하는 데이터를 담기 (request 의 attribute 영역에)
