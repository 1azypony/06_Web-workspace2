...
<%
	// 로그인한 사용자의 정보 뽑기 (session 으로부터)
	Member loginUser = (Member)session.getAttribute("loginUser");

	// System.out.println(loginUser);
	// 로그인 전 menubar.jsp 를 로딩 시 loginUser 에 null 이 담김
	// 로그인 후 menubar.jsp 를 로딩 시 loginUser 에 해당 회원의 정보가 담긴 Member 객체가 담김
	...
	// 로그인 서비스 요청 후 성공시 menubar.jsp 로딩 시 alert 로 띄워줄 메세지 문구
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<script>
		...
		if(msg != "null") {
			...
			// 알림창을 띄원 준 후 session 에 담긴 해당 메세지를 
			// session.removeAttribute("키값") 메소드로 지워줘야 함
			// 안그러면 menubar.jsp 가 로딩될때마다 매번 alert 가 계속 뜸
        ...
        <% if(loginUser == null) { %>
	        ...
	        <script>
	        	function enrollPage() {
	        		...
	        		// 단순한 정적인 페이지 요청이라고 해도 반드시 servlet 을 거쳐갈것!
	        		// => url 에 서블릿 맵핑값만 보이게끔 해줄것임
