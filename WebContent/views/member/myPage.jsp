...
<body>
    ...
    <%
    	...
    	String interest = (loginUser.getInterest() == null) ? "" : loginUser.getInterest(); // "운동, 등산, 게임"
    %>
        ...
        <form id="mypage-form" action="<%= contextPath %>/update.me" method="post">
            ...
            <script>
                $(function() {
                    ...
                    // 관심분야 checkbox 에 동적으로 체크될 수 있게끔 처리

                    var interest = "<%= interest %>"; // "" / "운동, 등산, 게임"
                    
                    // 순차적으로 checkbox 들에 접근 후 접근한 input 요소의 value 값이
                    // interest 라는 자바스크립트 변수에 포함되어있는지 검사
                    $("input[type=checkbox]").each(function() {

                        // search 메소드 : 해당 문자열에 매개변수로 넘긴 문자열이 포함되어있다면
                        //               해당 그 위치 인덱스값을 반환
                        //               아니라면 -1 값을 반환
                        //				 자바스크립트에서의 배열에서 indexOf 메소드와 유사한 역할
